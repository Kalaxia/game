...
readonly class ProductOfferIndexHandler
{
	...
	public function __invoke(ProductOfferIndexMessage $message): void
	{
		if (null === ($offerData = $this->productOfferRepository->get($message->productId))) {
			$this->logger->warning('Product offer {productId} either not found or cannot be produced', ['productId' => $message->productId]);
		...
		$this->logger->debug('Indexing product offer {productId}', ['productId' => $offer->productId]);
